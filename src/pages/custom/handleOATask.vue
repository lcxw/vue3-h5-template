...
import { SysFlowTaskOperationType } from '@/staticDict'
...
async function goToNext(): Promise<void> {
  ...
  try {
    const taskInfo = await SystemController.getTaskInfo({ todoId: todoId.value })
    if (taskInfo) {
      const params = {
 ...
 */
onMounted(() => {
  todoId.value = route.query.todoId as string || ''
